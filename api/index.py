"""Web App - Same sticker, silent ghosting, slow human chat (FULL)"""
...
class CR(BaseModel): message:str; gender:str="female"; name:str="Friend"; age:int=22; city:str="City"; country:str="Country"; flag:str="🌍"
@app.post("/api/chat")
async def api_chat(r:CR):
    t=r.message.strip()[:500]
    if not t: return JSONResponse({"error":"Empty"}, status_code=400)
...
<script>
const tg=window.Telegram?.WebApp;if(tg){tg.ready();tg.expand();tg.setHeaderColor('#FF007A');tg.setBackgroundColor('#0A0A12');}
...
let st='active',um=0,busy=false,us=false,h=[],ga=pG();const HK='wcv6_'+C.n,mc=$('mc'),inp=$('mi'),sb=$('sb');
...
async function api(t){const r=await fetch('/api/chat',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:t,gender:C.g,name:C.n,age:Number(C.a)||22,city:C.ci,country:C.co,flag:C.fl})});const d=await r.json();return d.ok&&d.reply?d.reply:"haha 😊";}
